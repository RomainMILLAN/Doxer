...
public class discordSecurity extends pluginSlashInterface {
                ...
                App.addPlugin(
                                new Plugin(
                                                ...
                                                new ArrayList<String>() {
                                                        {
                                                                add(DiscordSecurityMessages.PLUGIN_COMMAND_SECURITY
                                                                                .getMessage());
                                                                add(DiscordSecurityMessages.PLUGIN_COMMAND_CONFIRM
                                                                                .getMessage());
                                                        }
                                                }));
        }
                        ...
                        if (!MemberPermission.getInstance().isStaffMember(event.getMember())) {
                                event.replyEmbeds(
                                                ErrorCrafter.craftErrorPermission(
                                                                ...
                                                                MemberPermissionStates.STAFF).build())
                                                .setEphemeral(true)
                                                .queue(message -> {
                                ...
                                Sentry.getInstance().toLog(
                                                DiscordSecurityStates.PLUGIN_NAME.getState(),
                                                String.format(
                                                                SystemMessages.INCORRECT_PERMISSION_WITH_PERMISSION
                                                                                .getMessage(),
        ...
        private void confirmCommand(SlashCommandInteractionEvent event) {
                Member member = event.getOption(
                                DiscordSecurityStates.PLUGIN_OPTION_CONFIRM_USER_NAME.getState()).getAsMember();
                ...
                if (null != event.getOption(DiscordSecurityStates.PLUGIN_OPTION_CONFIRM_GROUP_NAME.getState())) {
                        Role role = event.getOption(
                                        DiscordSecurityStates.PLUGIN_OPTION_CONFIRM_GROUP_NAME.getState()).getAsRole();
                ...
                event.replyEmbeds(
                                DiscordSecurityCrafter.craftConfirmEmbed(member, event.getMember()).build()).queue();
                Sentry.getInstance().toLog(
                                DiscordSecurityStates.PLUGIN_NAME.getState(),
                                String.format(
        ...
        public void onGuildMemberJoin(GuildMemberJoinEvent event) {
                ...
                if (App.getConfiguration("ST_SECURITY").equals("TRUE")) {
                        TextChannel securityChannel = event.getGuild()
                                        .getTextChannelById(App.getConfiguration("TC_SECURITY"));
                        ...
                        Role defaultRole = event.getGuild()
                                        .getRoleById(App.getConfiguration("R_SECURITY_DEFAULT_GROUP"));
                        event.getGuild().addRoleToMember(event.getMember(), defaultRole).queue();

                        Sentry.getInstance().toLog(
                                        DiscordSecurityStates.PLUGIN_NAME.getState(),
                                        String.format(
                                                        DiscordSecurityMessages.SENTRY_SECURITY_FALSE_MEMBER_JOIN
                                                                        .getMessage(),
